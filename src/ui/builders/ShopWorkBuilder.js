...
export class ShopWorkBuilder {
  ...
  withValidDescription() {
    // уникальная метка в начале описания - по ней надежно проверяем,
    // что именно наш текст сохранился (не склеивается, легко найти на странице)
    const marker = `Продаю готовую работу ${faker.string.uuid()}`;
    let description = `${marker}. ${faker.lorem.words(60)}`
      .slice(0, 500)
      .trim();
