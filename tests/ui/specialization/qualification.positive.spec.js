...
// Предусловие: идет ПОСЛЕ регистрации (стартует под зарегистрированным пользователем,
// сессия registered-user.json; связка задана в конфиге).
...
test("Отправка формы квалификации: валидные данные + чекбокс -> запрос отправлен", async ({
  page,
  qualificationFacade,
}) => {
  ...
  await expect(qualificationFacade.specializationPage.infoAlert).toBeVisible();
  ...
  await expect(qualificationFacade.specializationPage.infoAlert).toContainText(
    "Ваш запрос принят",
  );
});
