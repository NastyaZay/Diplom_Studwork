...
- **Facade** — объединяет шаги страниц в готовые бизнес-сценарии;
- **Fixtures** — отдают в тест готовые фасады; применяются для подготовки и переиспользования объектов.
...
│   ├── constants/                # эталонные тексты ошибок API
...
> Тесты идут последовательно (`workers: 1`): UI-сценарии связаны цепочкой сессий, а API-кейсы портфолио зависят друг от друга (создаем → меняем → удаляем один и тот же объект).
...
**Результаты тестов прогона (16 из 16 зеленые):**
